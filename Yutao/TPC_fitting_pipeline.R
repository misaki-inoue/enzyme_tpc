...
# Check if every subset has the 'id' column and if each subset is not empty
for (i in 1:length(split_dataset)) {
  if (!"id" %in% names(split_dataset[[i]]) || nrow(split_dataset[[i]]) == 0) {
    cat("Dataset ", i, " is missing 'id' column or has no rows.\n", sep = '')
  }
}

# Initialize the results_summary data frame
results_summary <- data.frame(ID = rep(NA, length(split_dataset)), outcome = rep(NA, length(split_dataset)))
...
for (i in 1:length(split_dataset)) {
  if ("id" %in% names(split_dataset[[i]]) && nrow(split_dataset[[i]]) > 0) {
    ...
    results_summary$outcome[i] <- fit_models(split_dataset[[i]], split_dataset[[i]]$id[1])
    gc()
  } else {
    cat("Skipping dataset ", i, " due to missing 'id' or empty data.\n", sep = '')
  }
}
